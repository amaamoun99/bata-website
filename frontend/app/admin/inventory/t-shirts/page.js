...
export default function Products() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/v1/products?productType=t-shirt', {
          headers: {
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-center text-3xl font-bold text-white mb-6">T-shirt inventory</h1>
      {products.length > 0 ? (
